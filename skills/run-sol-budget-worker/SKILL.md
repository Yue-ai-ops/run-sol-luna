...
1. **One execution owner.** Luna Max or Sol owns the next action. An adviser or reviewer does not silently become a second executor.
2. **Luna Max is the default worker.** The valid Luna route is exactly `gpt-5.6-luna` with `max` reasoning.
3. **Sol is judgment bandwidth.** Use it for a consequential unresolved decision, a high-value plan, or one focused milestone review—not routine execution.
4. **Handoff preserves the outcome.** Keep completed work, decisions, constraints, unknowns, and the next action; never replace the user's goal with a smaller checklist.
5. **No duplicate ordinary work.** Recheck only a concrete high-risk conflict or required acceptance condition.
6. **Use the lowest adequate investment.** Max is for the hardest single tasks; Ultra is only for work that genuinely divides into independent streams.
...
8. **`TERRA_EXPLICIT`** — use Terra only when the user explicitly requests it.
...
If native Luna Max is stably rejected, use `LUNA_MAX_UNAVAILABLE`: do not retry for appearance, downgrade Luna, automatically route to Terra, or emulate a child with a second CLI process. The user may manually switch the task or authorize another route.

Use `TERRA_EXPLICIT` only after the user names Terra. Terra is not a hidden fallback in the Sol/Luna chain.
...
6. Terra remains explicit-only; model rejection is reported honestly.
