...
```text
省额度，Sol 主做，Astra 按需帮忙，独立小活交给 Luna Max。
```

这表示试用 Sol medium 持续执行，明确困难时再考虑 high；方向不清才请 Astra medium–xhigh 做必要方案，值得独立判断或明确要求时才请 Astra 窄审查。Luna Max 只承担独立、可验收的小块，不把每一步都往下分派。当前模型不会被 Skill 自动切换，接口不支持时如实说明。日常小活仍可直接用 Luna Max；尚无证据证明整条组合比单模型更省或更好。
